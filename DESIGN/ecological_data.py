...
if __name__ == '__main__':
    # Test spawn rates
    print("Spawn rates in Grassland:")
    for animal in ['rabbit', 'deer', 'bison', 'wolf', 'lion']:
        rate = calculate_spawn_rate(animal, 'grassland')
        print(f"  {animal}: {rate:.4f}")
    
    print("\nWolf ecological data:")
    wolf = ANIMALS['wolf']
    print(f"  Trophic level: {wolf['trophic_level']}")
    print(f"  Diet: {get_prey_for_predator('wolf')}")
    print(f"  Competes with: {get_competitors('wolf')}")
    
    print("\nDeer predators:")
    print(f"  {get_predators_for_prey('deer')}")
