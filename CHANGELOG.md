...
### Added
- **Culture System**:
  - **Culture Resource**: A new cumulative resource generated by your tribe.
  - **Generation**: +1 per living unit, +1 per active Bonfire, +2 per Shaman (Yearly).
  - **Bonfire Decay**: Bonfires now have 20 HP and decay by 1 HP per turn (lasting 5 years).
  - **Energy Restoration**: Bonfires restore 10 Energy to units within range 5 (Costs 1 Wood/turn).
  - **Recruitment Costs**: Recruiting a Crafter now requires 5 Culture in addition to Food.
- **Map Controls**: Added click-and-drag panning functionality to the World Map for easier navigation.
- **Structures**:
  - **Workshop**: New structure buildable by Crafters (Cost: 20 Wood, 20 Stone).
  - **Universal Bonfires**: Any unit class can now build Bonfires.

### Fixed
- **Hunt Interaction**:
  - Fixed a bug where clicking a target to hunt would trigger a "Move Confirmation" dialog, blocking the action.
  - Removed the browser `confirm()` dialog for the "Hunt" action, which was failing silently in some environments.
- **UI Updates**: Updated Stats Panel to show Culture income and new build options.
